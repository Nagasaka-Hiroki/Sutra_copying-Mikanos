...
namespace {
    class ConfigurationDescriptorReader {
        public:
            ...
            const T* Next() {
                while (auto n = Next()) {
                    if ( auto n = usb::DescriptorDynamicCast<T>(n)) {
                        retur d;
                    }
                }
                return nullptr;
        ...
        if(if_desc.interface_class == 3 &&
           if_desc.interface_sub_class == 1) {
            if(if_desc.interface_protocol == 1){
                auto keyboad_driver = new usb::HIDKeyboardDriver{dev, if_desc.interface_number};
                if (usb::HIDKeyboardDriver::default_observer) {
                    keyboard_driver->SubscribleKeyPush(usb::HIDKeyboardDriver::default_observer);
                }
                ...
                if(usb::HIDMouseDriver::default_obverser){
                    mouse_driver->SubscribleMouseMove(usb::HIDMouseDriver::default_observer);
                }
                return mouse_driver;
            }
        }
    ...
    void Log(LogLevel level, const usb::HIDDescroptor& hid_desc) {
...
namespace usb {
    ...
    Error Device::OnEndpointsConfigured() {
        for(auto class_driver : class_dirvers_){
    ...
    Error Device::OnControlCompleted(EndpointID ep_id, SetupData setup_data,
        ...
        Log(kDebug, "Device::OnControlComplete: buf 0x%08x, len %d, dir %d\n",
    ...
    Error Device::InitializePhase1(const uint8_t* buf, int len) {
        const auto device_desc_ = DescriptorDynamicCast<DeviceDescriptor>(buf);
    ...
    Error Device::InitializePhase2(const uint8_t* buf, int len) {
        ...
        while(auto if_desc= config_reader.Next<InterfaceDescroptor>()){
            Log(kDebug, *if_desc);
            ...
            while(num_ep_configs_ < if_desc->num_endpoints){
                ...
                if(auto ep_desc = DescriptorDynamicCast<EndpointDescriptor>(desc)){
                    ...
                    ep_configs_[num_ep_configss_] = conf;
                    ++num_ep_configs_;
